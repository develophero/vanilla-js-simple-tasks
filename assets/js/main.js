window.onload = function() {
  ...
  var taskModel = new window.TaskModel(this);
  var taskView = new window.TaskView(this);

  // ---------------------------------- Set Up
  var tasks = taskModel.getTasks();
  taskView.placeTasks(tasks);
  taskView.setUpNewTaskForm();

  // ---------------------------------- User Events 
  this.checkboxClickedAction = function(id, checked) {
    taskModel.checkboxClicked(id, checked);
  }

  // This happens when the save button is clicked
  this.newTaskClicked = function(taskContent) {
    var newTask = taskModel.newTask(taskContent);
    taskView.placeTask(newTask);
  }
